...
test("OR-set: country_code=US;CA", () => {
  ...
  assert.equal(q.quote({ sku: ".ng", quantity: 1, country: "US" }, "USD").salePrice, 800);
  assert.equal(q.quote({ sku: ".ng", quantity: 1, country: "CA" }, "USD").salePrice, 800);
  assert.equal(q.quote({ sku: ".ng", quantity: 1, country: "NG" }, "USD").salePrice, 1000);
});
...
test("negation: customer_tier=!=free", () => {
  ...
  assert.equal(free.lines[0].salePrice, 1000);
});

test("inclusive range: quantity=10..49", () => {
  const config = discountCatalog("quantity=10..49");
  const q = new Quotes(config);
  assert.equal(q.quote({ sku: ".ng", quantity: 9 }, "USD").salePrice, 1000);
  assert.equal(q.quote({ sku: ".ng", quantity: 10 }, "USD").salePrice, 800);
  assert.equal(q.quote({ sku: ".ng", quantity: 49 }, "USD").salePrice, 800);
  assert.equal(q.quote({ sku: ".ng", quantity: 50 }, "USD").salePrice, 1000);
});

test("threshold on line_subtotal (minor units, pre-adjustment)", () => {
  const config = discountCatalog("line_subtotal=>=10000");
  const q = new Quotes(config);
  assert.equal(q.quote({ sku: ".ng", quantity: 99 }, "USD").salePrice, 800); // 99*1000=99000 >= 10000
  assert.equal(q.quote({ sku: ".ng", quantity: 1 }, "USD").salePrice, 1000); // 1000 < 10000
});
...
test("AND across keys", () => {
  ...
  const q = new Quotes(config);
  assert.equal(q.quote({ sku: ".ng", quantity: 10, country: "US" }, "USD").salePrice, 800);
  assert.equal(q.quote({ sku: ".ng", quantity: 1, country: "US" }, "USD").salePrice, 1000);
  assert.equal(q.quote({ sku: ".ng", quantity: 10, country: "NG" }, "USD").salePrice, 1000);
});
...
test("a known field absent from the query fails the constraint without throwing", () => {
  ...
  assert.equal(r.lines[0].salePrice, 1000);
});
